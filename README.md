...
`with_defaults()` pre-populates a registry with the `file` and `mem`
drivers (in the latter's unscoped form). For server-side use, build an
empty registry and install a `FileScope` instead:
